...
\begin{figure}[h]
    ...
    \def\m{2} % M = 20 columns (front face width)
    \def\depth{3} % Depth of the cube (z-axis, number of cells in depth)
...
% \draw[->, thick] ({-\xoffset+(\m+1.5)*\cellsize}, {\n*\cellsize/2+\yoffset}) -- ({-\xoffset+(\m+4)*\cellsize}, {\n*\cellsize/2+\yoffset}) node[midway, above] {};

    % Add the shape label below the grid
    \node[below] at ({(-\xoffset+\m/2*\cellsize}, \yoffset+4.5*\cellsize) {\pyvar{t\_sid}};

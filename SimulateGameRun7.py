...
def find_utl(i_hnd, t_val, b_flg, i_sid, c_edg, t_edg, tr_sgm, tf_sgms, d0_rpr, d1_rpr, alg, TOL, i_cnt):
...
if __name__ == "__main__":
    ...
    def run_cfr(i_dck, LOG_TOL, I_TER):
        ...
        tr_sgm = {f't_sgm_{i_cod}': d_fls[f't_sgm_{i_cod}'].to(torch.float32) for i_cod in i_cods} #TODO:64
        tf_sgms  = {f't_sgm_{i_cod}': d_fls[f't_sgm_{i_cod}'].to(torch.float32) for i_cod in i_cods} #TODO:64
        ...
        for i_hnd in reversed(range(6)):
            ...
            t_rus  = d_fls[f't_rus_{i_hnd}'].to(torch.float32, device=device)  #TODO:64
